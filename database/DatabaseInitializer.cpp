...
bool DatabaseInitializer::createTriggers() {
    ...
    QStringList triggers = {
        ...
        R"(CREATE TRIGGER trg_track_circuit_edges_updated_at
        BEFORE UPDATE ON railway_control.track_circuit_edges
        FOR EACH ROW EXECUTE FUNCTION railway_control.update_timestamp())",

        R"(CREATE TRIGGER trg_signal_overlap_definitions_updated_at
...
bool DatabaseInitializer::createRolesAndPermissions() {
    ...
    if (success) {
        ...
        qWarning() << "⚠️ Some role/permission queries failed - this may be acceptable if roles already exist";
    }
...
bool DatabaseInitializer::populateRouteConfiguration() {
    qDebug() << "🔄 Populating route configuration data...";
    ...
    QVector<RouteConfigEntry> configEntries = {
        {"max_concurrent_routes", "10", "VITAL", "Maximum number of concurrent routes allowed"},
        {"pathfinding_timeout_ms", "500", "PERFORMANCE", "Maximum time allowed for pathfinding algorithm"},
        {"overlap_hold_default_seconds", "30", "VITAL", "Default overlap hold time in seconds"},
        {"route_validation_timeout_ms", "50", "VITAL", "Maximum time allowed for route validation"},
        {"resource_lock_timeout_minutes", "30", "OPERATIONAL", "Default timeout for resource locks"},
        {"degraded_mode_max_routes", "5", "VITAL", "Maximum routes in degraded mode"},
        {"performance_warning_threshold_ms", "45", "PERFORMANCE", "Performance warning threshold"},
        {"enable_route_visualization", "true", "OPERATIONAL", "Enable route visualization on UI"},
        {"enable_performance_monitoring", "true", "OPERATIONAL", "Enable performance metrics collection"},
        {"safety_violation_max_rate", "0.05", "VITAL", "Maximum allowed safety violation rate (5%)"}
    };

    QString query = R"(
        INSERT INTO railway_control.route_configuration
    ...
    if (!sqlQuery.prepare(query)) {
        setError(QString("Failed to prepare route configuration query: %1").arg(sqlQuery.lastError().text()));
        return false;
    }

    int successCount = 0;
    for (const auto& entry : configEntries) {
        sqlQuery.addBindValue(entry.key);
        sqlQuery.addBindValue(entry.value);
        sqlQuery.addBindValue(entry.type);
        sqlQuery.addBindValue(entry.description);

        if (!sqlQuery.exec()) {
            qWarning() << "Failed to insert route configuration entry:" << entry.key
        ...
        // Clear bind values for next iteration
        sqlQuery.finish();
    }

    if (successCount == 0) {
        setError("Failed to insert any route configuration entries");
        return false;
    }

    qDebug() << "✅ Successfully populated" << successCount << "of" << configEntries.size()
...
bool DatabaseInitializer::validateDatabase() {
    QStringList validationQueries = {
        ...
        "SELECT COUNT(*) FROM railway_control.route_configuration",
