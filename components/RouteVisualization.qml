...
Rectangle {
    ...
    function refreshRoutes() {
        ...
        console.log("🗺️ RouteVisualization: Refreshing active routes")
    ...
    function updateRouteOverlays() {
        console.log("🗺️ RouteVisualization: Updating route overlays for", activeRoutes.length, "routes")
    ...
    function createRouteOverlay(route) {
        ...
        console.log("🗺️ Creating route overlay for:", route.id, "state:", route.state)
        ...
        if (assignedCircuits.length === 0) {
            console.warn("⚠️ Route", route.id, "has no assigned circuits")
            return
        }
    ...
    function handleRouteRequested(requestId, sourceSignal, destSignal) {
        console.log("🗺️ Route requested:", requestId, sourceSignal, "→", destSignal)
    ...
    function handleRouteAssigned(routeId, sourceSignal, destSignal, path) {
        console.log("🗺️ Route assigned:", routeId, "path:", path)
        refreshRoutes()
    }
    
    function handleRouteActivated(routeId) {
        console.log("🗺️ Route activated:", routeId)
    ...
    function handleRouteReleased(routeId, reason) {
        console.log("🗺️ Route released:", routeId, "reason:", reason)
    ...
    function handleRouteFailed(routeId, reason) {
        console.log("🗺️ Route failed:", routeId, "reason:", reason)
    ...
    Component.onCompleted: {
        console.log("🗺️ RouteVisualization component initialized")
        ...
        if (routeAssignmentService) {
            ...
            console.warn("⚠️ RouteAssignmentService not available")
        }
    }
    
    Component.onDestruction: {
        clearAllOverlays()
        console.log("🗺️ RouteVisualization component destroyed")
    }
