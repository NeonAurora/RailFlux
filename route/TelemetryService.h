...
enum class MetricType {
    ...
    SAFETY,          // Safety violations, emergency actions
    OPERATIONAL,     // Route assignments, resource utilization
    SYSTEM          // System health, availability
};
...
struct PerformanceMetric {
    QString operation;           // "route_assignment", "pathfinding", "overlap_calculation"
    double responseTimeMs;
    QDateTime timestamp;
    bool success;
    QString context;             // Additional context (routeId, signalId, etc.)
    QVariantMap metadata;        // Flexible additional data
};
...
struct OperationalMetric {
    QString metricName;          // "active_routes", "resource_utilization", "success_rate"
    double value;
    QString unit;                // "count", "percentage", "seconds"
    QDateTime timestamp;
    QVariantMap dimensions;      // Tags for grouping (direction, signal_type, etc.)
};

struct SystemHealthMetric {
    QString component;           // "GraphService", "ResourceLockService", "OverlapService"
    QString healthStatus;        // "healthy", "degraded", "critical"
    double uptime;               // Uptime percentage
    QDateTime lastCheck;
    QVariantMap diagnostics;     // Component-specific health data
};
