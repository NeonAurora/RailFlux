...
enum class SafetyLevel {
    VITAL_SAFE,      // Highest safety level - route fully validated
    SAFE,            // Standard safety level - normal operations
    CAUTION,         // Caution required - potential conflicts detected
    WARNING,         // Warning level - safety concerns present
    DANGER           // Danger level - route not safe to use
};

enum class RouteState {
    REQUESTED,           // Initial state
    VALIDATING,         // Interlocking validation in progress
    RESERVED,           // Resources locked, signals not yet cleared
    ACTIVE,             // Route active, signals cleared for movement
    PARTIALLY_RELEASED, // Main route released, overlap still held
    RELEASED,           // Fully released
    FAILED,             // Validation/reservation failed
    EMERGENCY_RELEASED, // Force-released due to safety
