...
enum class ComplianceLevel {
    COMPLIANT,          // Fully compliant with all safety regulations
    MINOR_DEVIATION,    // Minor deviations that don't affect safety
    MAJOR_DEVIATION,    // Major deviations requiring attention
    SAFETY_CRITICAL,    // Safety-critical violations requiring immediate action
    NON_COMPLIANT      // Completely non-compliant
};

enum class ViolationType {
    ROUTE_CONFLICT,           // Multiple routes using same resources
    SIGNAL_VIOLATION,         // Invalid signal aspects/progression
    TRACK_CIRCUIT_VIOLATION,  // Track circuit safety violations
    POINT_MACHINE_VIOLATION,  // Point machine safety violations
    OVERLAP_VIOLATION,        // Overlap safety violations
    TIMING_VIOLATION,         // Safety timing requirements violated
    INTERLOCKING_VIOLATION,   // Interlocking rule violations
    OPERATOR_VIOLATION,       // Operator safety protocol violations
    SYSTEM_INTEGRITY,         // System integrity violations
    EMERGENCY_PROTOCOL       // Emergency protocol violations
};
